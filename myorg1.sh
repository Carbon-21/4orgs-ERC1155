...
function registerPerson() {
    fabric-ca-client register --caname ca-org1 --id.name $1 -d --id.secret $2 --id.type client --tls.certfiles "${PWD}/artifacts/channel/crypto-config/peerOrganizations/org1.example.com/tlsca/tlsca.org1.example.com-cert.pem"
    # Generate the identity certificates and MSP folder.
    fabric-ca-client enroll -u https://$1:$2@localhost:7054 -d --caname ca-org1 -M "${PWD}/artifacts/channel/crypto-config/peerOrganizations/org1.example.com/users/$1@org1.example.com/msp" --tls.certfiles "${PWD}/artifacts/channel/crypto-config/peerOrganizations/org1.example.com/tlsca/tlsca.org1.example.com-cert.pem"
...
function balanceOf() {
    ...
    do
        ...
        peer chaincode invoke "${TARGET_TLS_OPTIONS[@]}" -C mychannel -n ERC1155 -c "{\"function\":\"balanceOf\",\"Args\":[\"$P1\",\"$i\"]}"
    done
