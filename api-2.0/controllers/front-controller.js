...
exports.postSignup = async (req, res, next) => {
  ...
  axios
    ...
    .then(function (response) {
      ...
      res.json({success: true, token:response.data.token, certificate: response.data.certificate, swapRoleOrgOrder: response.data.swapRoleOrgOrder})
    })
...
exports.postLogin = async (req, res, next) => {
  ...
  axios
    ...
    .then(function (response) {
      ...
      req.session.username = email;
      req.session.role = JSON.parse(atob(response.data.token.split(".")[1])).role;
      res.json({success: true, token: response.data.token, keyOnServer: response.data.keyOnServer, swapRoleOrgOrder: response.data.swapRoleOrgOrder})
    })
